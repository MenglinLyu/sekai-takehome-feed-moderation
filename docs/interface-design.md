...
| `WebViewSlotPool` | Exactly three pool-owned `WKWebView`s. `mountWebViews(in:)` permanently mounts them in one canvas; `positionWebViews(itemFrames:parkedFrame:)` moves them only within that coordinate space. `assign(items:currentID:)`, `setEligibleTarget(_:)`, `removeHiddenItems(survivingIDs:)`, `presentation(for:)`, `retry(itemID:)`, and `memoryWarning()` manage bindings and state. One reconciliation task serializes pause, obsolete-binding reset, loads, and play. Navigation delegates validate WKNavigation identity. Failed pause resets the old document to blank before another slot can play. |
| `FeedCell` | `configure(item:pool:...)`, `render(_:)`, `cover()`. Renders transparent live state or opaque loading/error/moderation covers plus native creator/moderation controls. Passes non-control touches through to the canvas; never owns or reparents WebViews. |
| `FeedViewController` | Owns collection view, its permanent content-coordinate WebView canvas, diffable snapshots, current stable ID, settlement, and lifecycle. Covers removed cells synchronously, revokes playback, applies snapshots, positions the replacement and pooled views, then assigns the pool. Dragging only revokes eligibility; settlement updates assignments and resumes the ready target. |
